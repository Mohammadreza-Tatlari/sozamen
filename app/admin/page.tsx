  ...
  return (
    <div className="container">
      ...
      <div className="stat-grid">
        ...
        <div className="stat">
          محصولات<b>{products.length}</b>
        </div>
        <div className="stat">
          سفارش‌ها<b>{orders.length}</b>
        </div>
      </div>
      <section className="panel admin-section">
        ...
        {orders.length ? (
          orders.map((o) => (
            <div className="admin-order-row" key={o.id}>
              ...
              <div>
                ...
                <small>
                  {o.items.reduce((s, i) => s + i.quantity, 0)} کالا -{" "}
          ...
          <p className="empty">هنوز سفارشی ثبت نشده است.</p>
        )}
      </section>
      <section className="panel admin-section">
        <div className="section-head">
      ...
      <section className="panel admin-section admin-last">
        ...
        {customers.map((c) => (
          <div className="order" key={c.id}>
            <b>{c.name}</b>
            <p>
              {c.phone} - {c.address || "نشانی ثبت نشده"} - {c._count.orders} سفارش
            </p>
          </div>
        ))}
      </section>
