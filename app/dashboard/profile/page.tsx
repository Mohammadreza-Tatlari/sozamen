...
import { Camera, LogOut, PackageCheck, UserRound } from "lucide-react";
...
async function updateProfile(fd: FormData) {
  ...
  const name = String(fd.get("name") || "").trim(),
    ...
    image = fd.get("image");
  if (!name || !/^09\d{9}$/.test(phone)) redirect("/dashboard/profile?error=invalid");
  try {
    const profileImageUrl =
      image instanceof File && image.size ? await saveProfileImage(image) : undefined;
    await db.user.update({
      where: { id: user.id },
      data: { name, phone, address, ...(profileImageUrl && { profileImageUrl }) },
    });
  } catch {
    redirect("/dashboard/profile?error=duplicate");
  }
  redirect("/dashboard/profile?saved=1");
}
async function logout() {
  "use server";
  (await cookies()).delete("sozamen_session");
  redirect("/");
}
    ...
    <div className="container profile-layout">
      <aside className="panel profile-summary">
        <div className="profile-photo">
          {user.profileImageUrl ? (
            <img src={user.profileImageUrl} alt={`تصویر پروفایل ${user.name}`} />
          ) : (
            <UserRound size={44} />
          )}
        </div>
      ...
      <div>
        <section className="panel">
          ...
          {q.saved && <div className="success-notice">تغییرات پروفایل ذخیره شد.</div>}
          {q.error && (
            <div className="notice">
              {q.error === "duplicate"
                ? "این شماره موبایل قبلا استفاده شده است."
                : "نام و شماره موبایل معتبر وارد کنید."}
            </div>
          )}
          <form action={updateProfile}>
            <div className="field">
              <label>
                <Camera size={16} /> تصویر پروفایل
              </label>
