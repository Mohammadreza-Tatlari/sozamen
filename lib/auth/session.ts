...
export function makeSession(data: Session) {
  const p = Buffer.from(JSON.stringify(data)).toString("base64url");
  return `${p}.${sign(p)}`;
}
export function readSessionValue(value?: string): Session | null {
  try {
...
export async function getSession() {
  return readSessionValue((await cookies()).get("sozamen_session")?.value);
}
export async function getUser() {
  const s = await getSession();
  return s ? db.user.findUnique({ where: { id: s.id } }) : null;
}
